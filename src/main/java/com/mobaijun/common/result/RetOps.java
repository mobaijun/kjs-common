...
public class RetOps<T> {
     ...
     * 状态码为成功
     */
    public static final Predicate<R<?>> CODE_SUCCESS = r -> HttpStatus.OK.getCode() == r.getCode();

    /**
     * 数据有值
     */
    public static final Predicate<R<?>> HAS_DATA = r -> Objects.nonNull(r.getData());

    /**
     * 数据有值,并且包含元素
     */
    public static final Predicate<R<?>> HAS_ELEMENT = r -> Objects.nonNull(r.getData());
     ...
     *
     * @return 返回ture表示成功
     * @see com.mobaijun.common.enums.http.HttpStatus
     */
    public boolean isSuccess() {
        return codeEquals(HttpStatus.OK.getCode());
    }
    ...
    public <Ex extends Exception> RetOps<T> assertSuccess(Function<? super R<T>, ? extends Ex> func) throws Ex {
        return assertCode(HttpStatus.OK.getCode(), func);
    }
    ...
    public <Ex extends Exception> RetOps<T> assertDataNotEmpty(Function<? super R<T>, ? extends Ex> func) throws Ex {
        if (Objects.nonNull(original.getData())) {
            throw func.apply(original);
        }
        return this;
    }
    ...
    public <U> RetOps<U> map(Function<? super T, ? extends U> mapper) {
        R<U> result = R.ok(mapper.apply(original.getData()), original.getCode(), original.getMessage());
        return of(result);
    }
