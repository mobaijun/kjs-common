...
public class EnhanceImgUtil extends ImgUtil {
     ...
     * tools log
     */
    private static final Log log = Log.get(EnhanceImgUtil.class);
        ...
        if (FileUtil.isFile(image)) {
            return Optional.ofNullable(FileUtil.extName(image))
                    .map(fileExtName -> StrUtil.isBlank(fileExtName) || StrUtil.equalsAnyIgnoreCase(fileExtName,
                            EnhanceImgUtil.IMAGE_TYPE_JPG,
                            EnhanceImgUtil.IMAGE_TYPE_GIF, EnhanceImgUtil.IMAGE_TYPE_JPEG, EnhanceImgUtil.IMAGE_TYPE_BMP,
                            EnhanceImgUtil.IMAGE_TYPE_PNG, EnhanceImgUtil.IMAGE_TYPE_PSD))
                    .orElse(true);
        } else {
            return false;
        }
            ...
            if (NumberUtil.equals(Convert.toDouble(imageQuality), Convert.toDouble(1))) {
                ...
                try {
                    ...
                    if (StrUtil.equalsAnyIgnoreCase(FileUtil.extName(sourceImage), IMAGE_TYPE_JPG, IMAGE_TYPE_JPEG)) {
                        ...
                        EnhanceImgUtil.convert(IoUtil.toStream(sourceImage), ImgUtil.IMAGE_TYPE_JPG, jpgOutputStream);
                        bufferedImage = EnhanceImgUtil.toImage(jpgOutputStream.toByteArray());
                    }
